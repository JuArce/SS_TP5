    ...
    public static void main(String[] args) {
        ...
        humansQty.stream().parallel().forEach(hQty -> {
            IntStream.range(0, tries).forEach(i -> {
                ...
                Exporter csvExporter = new CsvExporter("output/ovito/", String.format("output_%s_%d.csv", hQty, i));
                csvExporter.open();
                Exporter velocityExporter = new VelocityExporter("output/velocity/", String.format("velocity_%s_%d.csv", hQty, i));
